...
/**
 * This filter is used by a {@link org.eclipse.jface.viewers.StructuredViewer <em>StructuredViewer</em>} to show all elements that are of <b>selectedClass</b>,
 * but exclude those that contain in a certain <b>attribute</b> a chosen <b>value</b>.
 * @author Andreea Olaru
 * @see {@link org.eclipse.eatop.app.semcon.allocationassistant.filters.TypeAndAttribureFilter <em>TypeAndAttribureFilter</em>}
 */
public class TypeAndExcludeByAttributeFilter extends ViewerFilter {
	...
	public TypeAndExcludeByAttributeFilter(EClass selectedClass, EStructuralFeature attribute, String value) {
		super();
	 ...
	 * does not contain any of the words from <b>value</b>. The comparison is not case-sensitive.
	...
	private boolean isSelectedElement(Object element){
		...
		if (selectedClass.isInstance(element) ) {
			if (attribute != null){
				...
				if (value.isEmpty()) 
					// if the value is empty it doesn't mean that the element should be excluded,
					// even though theoretically any string contains an empty string.
					doesntContain = true;
