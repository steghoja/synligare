 ...
 * 
 */
package org.eclipse.eatop.examples.graphicaleditor.depald.features;
...
public class FunctionProtoTypeLayout extends AbstractLayoutFeature {
	...
	public static final int MIN_CONTAINER_WIDTH = MIN_WIDTH + 10;
	/** minimal height for the port container shapes. */
	public static final int MIN_CONTAINER_HEIGHT = MIN_HEIGHT + 10;

	public FunctionProtoTypeLayout(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canLayout(ILayoutContext context) {
		...
		return businessObjects.get(0) instanceof AnalysisFunctionPrototype;
	}
	...
	public boolean layout(final ILayoutContext context) {
		...
		int containerWidth = containerGa.getWidth() - 10;
		int containerHeight = containerGa.getHeight() - 10;
		...
		for (Shape shape : containerShape.getChildren()) {
			...
			if (containerWidth != size.getWidth()) {
				...
				if (graphicsAlgorithm instanceof Polyline) {
					...
					Point newSecondPoint = gaService.createPoint(containerWidth + 10 / 2, secondPoint.getY());
		...
		for (GraphicsAlgorithm child : containerGa.getGraphicsAlgorithmChildren()) {
			gaService.setLocationAndSize(child, 10 / 2, 10 / 2, containerWidth, containerHeight);
			anythingChanged = true;
		}
