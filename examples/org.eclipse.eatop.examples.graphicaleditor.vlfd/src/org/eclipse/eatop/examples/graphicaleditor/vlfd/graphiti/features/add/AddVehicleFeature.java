...
public class AddVehicleFeature extends AbstractAddShapeFeature {
	private static final IColorConstant VehicleFeature_TEXT_FOREGROUND = IColorConstant.BLACK;

	private static final IColorConstant VehicleFeature_FOREGROUND = new ColorConstant(
			0, 0, 139);

	private static final IColorConstant VehicleFeature_BACKGROUND = new ColorConstant(
			0, 191, 255);

	public AddVehicleFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public PictogramElement add(IAddContext context) {
		final VehicleFeature addedQualReq = (VehicleFeature) context
				.getNewObject();
...

		{
			...
			roundedRectangleQR = gaService.createRoundedRectangle(
					containerShape, 5, 5);
			roundedRectangleQR
					.setForeground(manageColor(VehicleFeature_FOREGROUND));
			roundedRectangleQR
					.setBackground(manageColor(VehicleFeature_BACKGROUND));
		...
		// SHAPE WITH LINE
		{
			...
			Polyline polyline = gaService.createPolyline(
					(GraphicsAlgorithmContainer) shape, new int[] { 0, 25,
							width, 25 });
			polyline.setForeground(manageColor(VehicleFeature_FOREGROUND));
		...
		// SHAPE WITH ShortName
		{
			...
			text.setForeground(manageColor(VehicleFeature_TEXT_FOREGROUND));
		...
		// SHAPE WITH TEXT for QualityRequirementType
		{
			...
			Text text1 = gaService.createText(shape, addedQualReq.eClass()
					.getName());
			text1.setForeground(manageColor(VehicleFeature_TEXT_FOREGROUND));
