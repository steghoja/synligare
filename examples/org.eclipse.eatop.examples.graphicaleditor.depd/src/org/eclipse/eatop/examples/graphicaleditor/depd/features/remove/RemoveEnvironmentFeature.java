...
public class RemoveEnvironmentFeature extends DefaultRemoveFeature {

	public RemoveEnvironmentFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public void preRemove(IRemoveContext context) {
		...
		Assert.isTrue(getBusinessObjectForPictogramElement(element.getEnd().getParent()) instanceof OperationalSituation, "The start element isn't a OperationalSituation!");
		OperationalSituation target = (OperationalSituation) getBusinessObjectForPictogramElement(element.getEnd().getParent());
		
		EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getHazardousEvent_Environment();
