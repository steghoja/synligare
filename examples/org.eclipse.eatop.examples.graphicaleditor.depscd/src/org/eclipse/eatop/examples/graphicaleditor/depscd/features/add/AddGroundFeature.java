 ...
 * 
 */
package org.eclipse.eatop.examples.graphicaleditor.depscd.features.add;
...
public class AddGroundFeature extends AbstractAddShapeFeature {
	...
	public AddGroundFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
        ...
        if (context.getNewObject() instanceof Ground) {
	...
	public PictogramElement add(IAddContext context) {
		Ground addedGround = (Ground) context.getNewObject();
...

		{
			...
			if (addedGround.eResource() == null) {
				getDiagram().eResource().getContents().add(addedGround);
			}
			// create link and wire it
			link(containerShape, addedGround);
		}
		...
		// SHAPE WITH TEXT for ShortName
		{
			...
			Text text = gaService.createText(shape, addedGround.getShortName());
		...
		// SHAPE WITH TEXT for ItemType
		{
			...
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text
			Text text1 = gaService.createText(shape, addedGround.eClass()
					.getName());
