...
public class AddClaimFeature extends AbstractAddShapeFeature {
	...
	public AddClaimFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
        ...
        if (context.getNewObject() instanceof Claim) {
	...
	public PictogramElement add(IAddContext context) {
		Claim addedClaim = (Claim) context.getNewObject();
...

		{
			...
			if (addedClaim.eResource() == null) {
				getDiagram().eResource().getContents().add(addedClaim);
			}
		...
		// SHAPE WITH TEXT for ItemType
		{
			...
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text
			Text text1 = gaService.createText(shape, addedClaim.eClass()
					.getName());
