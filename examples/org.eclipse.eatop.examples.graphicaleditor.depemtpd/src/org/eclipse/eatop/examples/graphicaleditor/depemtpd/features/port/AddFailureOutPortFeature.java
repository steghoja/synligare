...
public class AddFailureOutPortFeature extends AbstractAddShapeFeature {
	...
	{
		FailureOutPort portToAdd = (FailureOutPort) context.getNewObject();
		...
		else 
		{
			...
			EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getErrorModelType_Failure();
	...
	private PictogramElement createBoundPort(final ContainerShape container, final int xpos, final int ypos, IAddContext context) 
	{
		...
		
		{
			...
			final Text text = gaService.createPlainText(shape, "FOP  : "+ FailureOutPort.getShortName());
