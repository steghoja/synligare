...
public class AddFaultInPortFeature extends AbstractAddShapeFeature {
	...
	public AddFaultInPortFeature(IFeatureProvider fp) {
	...
	public boolean canAdd(IAddContext context) 
	{
		...
		if (context.getNewObject() instanceof FaultInPort) {
        	...
        	if (context.getTargetContainer().getLink().getBusinessObjects()
    				.get(0) instanceof ErrorModelType
    				&& context.getNewObject() instanceof FaultFailurePort) {
	...
	{
		FaultInPort portToAdd = (FaultInPort) context.getNewObject();
		...
		else 
		{
			...
			////Literals.FAULT_IN_PORT;
			//ErrormodelPackage.Literals.ERROR_MODEL_TYPE__EXTERNAL_FAULT;
			EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getErrorModelType_ExternalFault();
	...
	private PictogramElement createBoundPort(final ContainerShape container, final int xpos, final int ypos, IAddContext context) 
	{
		...
		FaultInPort FaultInPort = (FaultInPort) context.getNewObject();
		...
		
		{
			...
			final Text text = gaService.createPlainText(shape, "FIP  : "+ FaultInPort.getShortName());
