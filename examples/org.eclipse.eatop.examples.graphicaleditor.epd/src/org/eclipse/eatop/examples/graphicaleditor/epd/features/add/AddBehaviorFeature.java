...
public class AddBehaviorFeature extends AbstractAddShapeFeature{
	...
	public AddBehaviorFeature(IFeatureProvider fp) {
		super(fp);
	}
	@Override
	public boolean canAdd(IAddContext context) {
		if (context.getNewObject() instanceof Behavior) {
	...
	public PictogramElement add(IAddContext context) {
		Behavior addedBehavior = (Behavior) context.getNewObject();
...

		{
			...
			if (addedBehavior.eResource() == null) {
				getDiagram().eResource().getContents().add(addedBehavior);
			}
			link(containerShape, addedBehavior);
		}
...

		{
			...
			Text text = gaService.createText(shape, addedBehavior.getShortName());
		...
		{
			Shape shape = (Shape) peCreateService.createShape(containerShape, false);
			Text text1 = gaService.createText(shape, addedBehavior.eClass().getName());
