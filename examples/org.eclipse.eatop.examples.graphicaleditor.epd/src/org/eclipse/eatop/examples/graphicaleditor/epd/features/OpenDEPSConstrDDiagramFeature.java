...
public class OpenDEPSConstrDDiagramFeature extends AbstractDrillDownFeature {
	private IPeService peService;
	private Collection<Diagram> diagrams;
	
	public OpenDEPSConstrDDiagramFeature(IFeatureProvider fp) {
		super(fp);
	...
	private void findAllDiagrams() {
		...
		TreeIterator<EObject> iter = this.getDiagram().eResource().getAllContents();
		while (iter.hasNext()) {
	...
	public boolean canExecute(ICustomContext context) {
		...
		if (pictogramElements != null && pictogramElements.length == 1) {
			...
			if (bo instanceof Dependability) {
				...
				if(dep.getName().equals("DependabilitySafetyConstraints")){
				return true;
				}
			}
		}
	...
	public void execute(ICustomContext context) {
		...
		if (elements.length == 1) {
			...
			EObject clickedBusinessObject = clickedPictogramElement.getLink().getBusinessObjects().get(0);

			final Collection<Diagram> linkedDiagrams = getLinkedDiagrams(clickedPictogramElement);
			if (linkedDiagrams.size() == 0) {
				//there is no diagram associated with the element. Create a new one!
				Diagram diagram = peService.createDiagram("org.eclipse.eatop.examples.graphicaleditor.depsconstrd", "DependabilitySafetyConstraints diagram", true);
