...
public class CreateEnvironmentFeature extends AbstractCreateFeature {
	public static String CREATE_CONNECTION_NAME = "Environment"; //$NON-NLS-1$
	public static String CREATE_CONNECTION_DESCRIPTION = "Create Environment"; //$NON-NLS-1$
	public static final String TITLE = "Create Environment"; //$NON-NLS-1$
	public static final String USER_QUESTION = "Enter name"; //$NON-NLS-1$ //$NON-NLS-2$
	public CreateEnvironmentFeature(IFeatureProvider fp) {
		super(fp, CREATE_CONNECTION_NAME, CREATE_CONNECTION_DESCRIPTION);
	}

	@Override
	public String getCreateImageId() {
	return EPDImageProvider.IMAGE_ENVIRONMENT;
	...
	public Object[] create(ICreateContext context) {
		...
		EAPackage newEnvironment = Eastadl21Factory.eINSTANCE.createEAPackage();
		newEnvironment.setName("EnvironmentPackage");
		newEnvironment.setShortName(userAskedString);
		...
		String fragment = EcoreUtil.getURI(newEnvironment).fragment();
		if (DiagramUtil.getEObject(parentBO, fragment) != null) {
			// the object already exist (Drag & Drop)
		} else {
			DiagramUtil.addObjectToBOResource(parentBO, referenceId, newEnvironment);
		}		
				
		addGraphicalRepresentation(context, newEnvironment);
		return new Object[] { newEnvironment };		
	}
