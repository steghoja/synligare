...
public class OpenHCTPDiagramFeature extends AbstractDrillDownFeature {
	private IPeService peService;
	private Collection<Diagram> diagrams;
	public OpenHCTPDiagramFeature(IFeatureProvider fp) {
		super(fp);
		...
		if (elements.length == 1) {
			...
			if (linkedDiagrams.size() == 0) {
				...
				Diagram diagram = peService.createDiagram(
						"org.eclipse.eatop.examples.graphicaleditor.hctpd", "HardwareComponentTypePackage diagram",
						true);
