...
public class OpenEPDDiagramFeature extends AbstractDrillDownFeature {

	private IPeService peService;
	private Collection<Diagram> diagrams;

	public OpenEPDDiagramFeature(IFeatureProvider fp) {
		super(fp);
	...
	public boolean canExecute(ICustomContext context) {
		...
		if (pictogramElements != null && pictogramElements.length == 1) {
			...
			EAPackage ea=(EAPackage) bo;
			if (ea.getName().equals("")) {	
			return true;		
			}
		}
		}
	...
	public void execute(ICustomContext context) {
		...
		if (elements.length == 1) {
			...
			if (linkedDiagrams.size() == 0) {
				...
				Diagram diagram = peService.createDiagram("org.eclipse.eatop.examples.graphicaleditor.epd", "EAPackage diagram", true);
