...
public class OpenDEPEMTPDFeature extends AbstractDrillDownFeature {

	private IPeService peService;
	private Collection<Diagram> diagrams;

	public OpenDEPEMTPDFeature(IFeatureProvider fp)
	{
		super(fp);
	...
	public boolean canExecute(ICustomContext context) 
	{
		...
		if (pictogramElements != null && pictogramElements.length == 1) 
		{
			...
			if (bo instanceof Dependability) 
			{
				...
				if(dep.getName().equals("DependabilityErrorModelTypePackage")){
				return true;
				}
			}
		}
	...
	public void execute(ICustomContext context) 
	{
		...
		if (elements.length == 1) 
		{
			...
			if (linkedDiagrams.size() == 0) 
			{
				...
				Diagram diagram = peService.createDiagram("org.eclipse.eatop.examples.graphicaleditor.depemtpd", "DependabilityErrorModelTypePackage diagram", true);
