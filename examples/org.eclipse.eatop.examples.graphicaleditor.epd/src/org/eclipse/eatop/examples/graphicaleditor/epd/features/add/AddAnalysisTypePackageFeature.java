...
public class AddAnalysisTypePackageFeature extends AbstractAddShapeFeature {
	...
	public AddAnalysisTypePackageFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public PictogramElement add(IAddContext context) {
		EAPackage addedEAPackage = (EAPackage) context.getNewObject();
...

		{
			...
			if (((EObject) addedEAPackage).eResource() == null) {
				getDiagram().eResource().getContents().add((EObject) addedEAPackage);
			}
			link(containerShape, addedEAPackage);
		}
...

		{
			...
			Text text = gaService.createText(shape, addedEAPackage.getShortName());
		...
		{
			Shape shape = (Shape) peCreateService.createShape(containerShape, false);
			Text text1 = gaService.createText(shape, addedEAPackage.getName());
