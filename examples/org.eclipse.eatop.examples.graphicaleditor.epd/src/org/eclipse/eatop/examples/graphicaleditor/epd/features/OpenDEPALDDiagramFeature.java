...
public class OpenDEPALDDiagramFeature extends AbstractDrillDownFeature {
	private IPeService peService;
	private Collection<Diagram> diagrams;
	
	public OpenDEPALDDiagramFeature(IFeatureProvider fp) {
		super(fp);
		...
		if (elements.length == 1) {
			...
			if (linkedDiagrams.size() == 0) {
				...
				Diagram diagram = peService.createDiagram(
						"org.eclipse.eatop.examples.graphicaleditor.depald", "DependabilityAnalysisLevel diagram",
						true);
	...
	public boolean canExecute(ICustomContext context) {
		...
		if (pictogramElements != null && pictogramElements.length == 1) {
			...
			if (bo instanceof Dependability) {
				...
				if(dep.getName().equals("DependabilityAnalysisLevel")){
					return true;
				}
