...
public class OpenATPDDiagramFeature extends AbstractDrillDownFeature {
	private IPeService peService;
	private Collection<Diagram> diagrams;
	public OpenATPDDiagramFeature(IFeatureProvider fp) {
		super(fp);
		...
		if (elements.length == 1) {
			...
			if (linkedDiagrams.size() == 0) {
				...
				Diagram diagram = peService.createDiagram(
						"org.eclipse.eatop.examples.graphicaleditor.atpd", "AnalysisTypePackage diagram", true);
	...
	public boolean canExecute(ICustomContext context) {
		...
		if (pictogramElements != null && pictogramElements.length == 1) {
			...
			if (bo instanceof EAPackage) {
				EAPackage dep=(EAPackage) bo;
				if(dep.getName().equals("AnalysisTypePackage")){
					return true;
				}
			}
		}
