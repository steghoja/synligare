 ...
 * 
 */
package org.eclipse.eatop.examples.graphicaleditor.atpd.features.add;
...
public class AddFunctionInFlowPortFeature extends AbstractAddShapeFeature {
	...
	public AddFunctionInFlowPortFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
		...
		if (context.getNewObject() instanceof FunctionFlowPort) {
        	...
        	if (context.getTargetContainer().getLink().getBusinessObjects()
    				.get(0) instanceof FunctionType
    				&& context.getNewObject() instanceof FunctionPort) {
    			ContainerShape parentC = context.getTargetContainer()
    					.getContainer();
    			for (Shape s : parentC.getChildren())
    				for (Anchor cs : s.getAnchors()) {
    					if (getBusinessObjectForPictogramElement(cs) instanceof FunctionPort) {
	...
	private PictogramElement createBoundPort(final ContainerShape container, final int xpos, final int ypos , IAddContext context) {
		...
		FunctionFlowPort functionFlowPort = (FunctionFlowPort) context.getNewObject();
		...
		
		{
			...
			final Text text = gaService.createPlainText(shape, direction +" : "+ functionFlowPort.getShortName());
