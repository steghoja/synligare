 ...
 * 
 */
package org.eclipse.eatop.examples.graphicaleditor.emtd.features.port;
...
public class AddFailureOutPortFeature extends AbstractAddShapeFeature {
	public String direction;
	public static final int PORT_SIZE = 10;
	private static final IColorConstant E_REFERENCE_FOREGROUND = new ColorConstant(
			98, 131, 167);

	public AddFailureOutPortFeature(IFeatureProvider fp) {
	...
	public boolean canAdd(IAddContext context) {
		// check if user wants to add a FailureOutPort
		if (context.getNewObject() instanceof FailureOutPort) {
			...
			// free flying ports
			for (Anchor cs : context.getTargetContainer().getAnchors()) {
				if (getBusinessObjectForPictogramElement(cs) instanceof FaultFailurePort) {
					if (getBusinessObjectForPictogramElement(cs).equals(
	...
	private PictogramElement createBoundPort(final ContainerShape container,
		...
		FailureOutPort FailureOutPort = (FailureOutPort) context.getNewObject();
