...
public class AddCommunicationHardwarePinFeature extends AbstractAddShapeFeature {
	public String direction;
	public static final int PORT_SIZE = 10;
	private static final IColorConstant E_REFERENCE_FOREGROUND = new ColorConstant(
			98, 131, 167);

	public AddCommunicationHardwarePinFeature(IFeatureProvider fp) {
	...
	public boolean canAdd(IAddContext context) {
		...
		if (context.getNewObject() instanceof CommunicationHardwarePin) {
	...
	public PictogramElement add(IAddContext context) {
		CommunicationHardwarePin portToAdd = (CommunicationHardwarePin) context
				.getNewObject();
		...
		// Does the context come from Drag & Drop ? see:
		// BasicGraphitiObjectTransferDropTargetListener
		if (context.getProperty("container") != null) { //$NON-NLS-1$
			...
			EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getHardwareComponentType_Pin();
			...
			if (!(context.getTargetContainer() instanceof Diagram)) {
				newElement = createBoundPort(context.getTargetContainer(),
						context.getX(), context.getY(), context);
				link(newElement, context.getNewObject());
			}
		}
	...
	private PictogramElement createBoundPort(final ContainerShape container,
			final int xpos, final int ypos, IAddContext context) {
		...
		CommunicationHardwarePin functionFlowPort = (CommunicationHardwarePin) context
				.getNewObject();
		IGaService gaService = Graphiti.getGaService();
		if (functionFlowPort.getDirection() == EADirectionKind.IN) {
			direction = "CHP : I";
		}
		if (functionFlowPort.getDirection() == EADirectionKind.OUT) {
			direction = "CHP : O";
		}
		if (functionFlowPort.getDirection() == EADirectionKind.INOUT) {
			direction = "CHP : IO";
		}

		{
			...
			final Shape shape = peCreateService.createShape(
					context.getTargetContainer(), true);

			// create and set text graphics algorithm
			final Text text = gaService.createPlainText(shape, direction
					+ " : " + functionFlowPort.getShortName());
			text.setStyle(StyleUtil.getStyleForEClassText(getDiagram()));
			gaService.setLocationAndSize(text, context.getX(), context.getY(),
					60, 20);
			...
			directEditingInfo.setMainPictogramElement(context
					.getTargetContainer());
