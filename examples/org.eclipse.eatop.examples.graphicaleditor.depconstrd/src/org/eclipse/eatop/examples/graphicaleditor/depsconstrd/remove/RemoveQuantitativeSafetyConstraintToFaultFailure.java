...
public class RemoveQuantitativeSafetyConstraintToFaultFailure extends DefaultRemoveFeature {

	public RemoveQuantitativeSafetyConstraintToFaultFailure(IFeatureProvider fp) {
		super(fp);
	}
	...
	public void preRemove(IRemoveContext context) {
		...
		Assert.isTrue(getBusinessObjectForPictogramElement(element.getStart().getParent()) 
				instanceof QuantitativeSafetyConstraint, "The start element isn't a QuantitativeSafetyConstraint!");
		QuantitativeSafetyConstraint source = (QuantitativeSafetyConstraint) getBusinessObjectForPictogramElement(element.getStart().getParent());
		...
		EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getErrorBehavior_ExternalFailure();
