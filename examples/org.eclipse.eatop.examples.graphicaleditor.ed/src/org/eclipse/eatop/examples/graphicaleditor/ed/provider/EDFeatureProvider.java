...
import org.eclipse.eatop.examples.graphicaleditor.ed.element.AddAnalysisFunctionPrototypeFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.element.AddDesignFunctionPrototypeFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.element.AddEnvironmentFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.element.CreateEnvironmentFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.features.DeleteFunctionPortsFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.features.EDLayoutFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.features.EDUpdateFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.features.MovePortsFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.instanceref.AddClampConnectorFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.instanceref.CreateClampConnectorFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.port.AddFunctionClientServerPortFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.port.AddFunctionInFlowPortFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.port.AddFunctionPowerPortFeature;
import org.eclipse.eatop.examples.graphicaleditor.ed.remove.RemoveClampConnector;
...
public class EDFeatureProvider extends DefaultFeatureProvider {

	public EDFeatureProvider(IDiagramTypeProvider dtp) {
		super(dtp);
	}
	...
	public IAddFeature getAddFeature(IAddContext context) {
		...
		if (context.getNewObject() instanceof Environment) {
		...
		if (context instanceof AddConnectionContext) {
			...
			if (sourceObject instanceof FunctionPrototype
					&& targetObject instanceof FunctionPrototype) {
				return new AddClampConnectorFeature(this);
			}
		}
		return super.getAddFeature(context);
	}
	...
	public IRemoveFeature getRemoveFeature(IRemoveContext context) {
		...
		if (pictogramElement instanceof Connection) {
			if (((Connection) pictogramElement).getStart().getLink() != null
					&& ((Connection) pictogramElement).getEnd().getLink() != null) {
				// only for Ports
				if (((Connection) pictogramElement).getStart().getLink().getBusinessObjects().get(0)
								instanceof FunctionPort
						&& ((Connection) pictogramElement).getEnd().getLink()
								.getBusinessObjects().get(0) instanceof FunctionPort) {
					return new RemoveClampConnector(this);
				}
			}
		}
	...
	public IUpdateFeature getUpdateFeature(IUpdateContext context) {
		...
		if (pictogramElement instanceof ContainerShape) {
	       ...
	       if (bo instanceof AnalysisFunctionPrototype ||bo instanceof DesignFunctionPrototype) {
	           return new EDUpdateFeature(this);
	       }
	   }
	...
	public ILayoutFeature getLayoutFeature(ILayoutContext context) {
		    ...
		    if (bo instanceof DesignFunctionPrototype
		        ...
		        return new EDLayoutFeature(this);
		    }
	...
	public ICreateConnectionFeature[] getCreateConnectionFeatures() {
		return new ICreateConnectionFeature[] {
				new CreateClampConnectorFeature(this)
		};
	}
