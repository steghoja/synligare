...
public class AddOperationalSituationFeature extends AbstractAddShapeFeature {
	...
	public AddOperationalSituationFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public boolean canAdd(IAddContext context) {
		...
		if (context.getNewObject() instanceof OperationalSituation) {
	...
	public PictogramElement add(IAddContext context) {
		final OperationalSituation addedOperSit = (OperationalSituation) context
				.getNewObject();
		// final Requirement addedClass = (Requirement) context.getNewObject();
...

		{
			...
			if (((EObject) addedOperSit).eResource() == null) {
				getDiagram().eResource().getContents().add((EObject) addedOperSit);
			}
			// create link and wire it
			link(containerShape, addedOperSit);
		}
		...
		// SHAPE WITH TEXT for ShortName
		{
			...
			Text text = gaService
					.createText(shape, addedOperSit.getShortName());
		...
		// SHAPE WITH TEXT for OperationalSituationType
		{
			// create shape for text
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text
			Text text1 = gaService.createText(shape, ((EObject) addedOperSit).eClass()
					.getName());
		...
		{
			// create shape for TextOperationalSituation
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text 2
			Text text2 = gaService.createText(shape, addedOperSit.getText());
