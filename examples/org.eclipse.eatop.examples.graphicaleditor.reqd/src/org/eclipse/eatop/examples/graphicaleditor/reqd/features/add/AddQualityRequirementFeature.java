...
public class AddQualityRequirementFeature extends AbstractAddShapeFeature {
	private static final IColorConstant QRequirement_TEXT_FOREGROUND = IColorConstant.BLACK;
	private static final IColorConstant QRequirement_FOREGROUND = new ColorConstant(0, 0, 139);
	private static final IColorConstant QRequirement_BACKGROUND = new ColorConstant(0, 191, 255);
	public AddQualityRequirementFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	public PictogramElement add(IAddContext context) {
		final QualityRequirement addedQualReq = (QualityRequirement) context
				.getNewObject();
...

		{
			...
			if (((EObject) addedQualReq).eResource() == null) {
				getDiagram().eResource().getContents().add((EObject) addedQualReq);
			}
		...
		// SHAPE WITH TEXT for QualityRequirementType
		{
			...
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text
			Text text1 = gaService.createText(shape, ((EObject) addedQualReq).eClass()
					.getName());
		...
		// SHAPE WITH TEXT for TextQualityRequirement
		{
			// create shape for text
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text 2
			Text text2 = gaService.createText(shape, addedQualReq.getText());
		...
		// SHAPE WITH TEXT for QualityRequirementKind
		{
			// create shape for text
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text 3
			Text text3 = gaService.createText(shape, addedQualReq.getKind()
					.toString());
			text3.setForeground(manageColor(QRequirement_TEXT_FOREGROUND));
			text3.setHorizontalAlignment(Orientation.ALIGNMENT_LEFT);
			text3.setFont(gaService
					.manageDefaultFont(getDiagram(), true, false));
            ...
            //direct editing shall be opened after object creation
            directEditingInfo.setPictogramElement(shape);
            directEditingInfo.setGraphicsAlgorithm(text3);
			// create link and wire it
			link(shape, addedQualReq);
