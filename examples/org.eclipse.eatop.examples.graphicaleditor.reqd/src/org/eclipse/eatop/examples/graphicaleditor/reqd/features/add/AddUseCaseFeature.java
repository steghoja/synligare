...
public class AddUseCaseFeature extends AbstractAddShapeFeature {
	...
	public PictogramElement add(IAddContext context) {
		final UseCase addedUseCase = (UseCase) context.getNewObject();
...

		{
			...
			if (((EObject) addedUseCase).eResource() == null) {
				getDiagram().eResource().getContents().add((EObject) addedUseCase);
			}
			// create link and wire it
			link(containerShape, addedUseCase);
		}
		...
		// SHAPE WITH TEXT for ShortName
		{
			...
			Text text = gaService
					.createText(shape, addedUseCase.getShortName());
		...
		// SHAPE WITH TEXT for UseCaseType
		{
			// create shape for text
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text
			Text text1 = gaService.createText(shape, ((EObject) addedUseCase).eClass()
					.getName());
		...
		// SHAPE WITH TEXT for UseCaseText
		{
			// create shape for TextUseCase
			Shape shape = (Shape) peCreateService.createShape(containerShape,
					false);
			// add Text 2
			Text text2 = gaService.createText(shape, addedUseCase.getText());
