...
public class AddAnalysisFunctionPrototypeFeature extends
	...
	public AddAnalysisFunctionPrototypeFeature(IFeatureProvider fp) {
	...
	public boolean canAdd(IAddContext context) {
		if (context.getNewObject() instanceof AnalysisFunctionPrototype) 
		{
						...
						.get(0) instanceof AnalysisLevel) 
		 {
					...
					AnalysisLevel targetfm = (AnalysisLevel) context.getTargetContainer()
							.getLink().getBusinessObjects().get(0);
					// Any AnalysisLevel contains container of new AnalysisFunctionPrototype
					// Systemmodels
					for (Shape cs : context.getTargetContainer().getContainer()
						...
						for (Shape cs2 : ((ContainerShape) cs).getChildren()) {
							if (cs2.getLink() != null) {
								if (cs2.getLink().getBusinessObjects().get(0) instanceof AnalysisLevel) {
									// AnalysisFunctionPrototype
									for (Shape cs3 : ((ContainerShape) cs2)
											.getChildren()) {
										if (cs3.getLink() != null) {
											if (cs3.getLink().getBusinessObjects()
													.get(0) instanceof AnalysisFunctionPrototype) {
					...
					// Any AnalysisLevel contains business object of new AnalysisFunctionPrototype
					// Systemmodels
					for (Shape cs : context.getTargetContainer().getContainer()
							.getContainer().getChildren()) {
						// AnalysisLevel
						...
						if(cs2.getLink() != null){
						if (cs2.getLink().getBusinessObjects().get(0) instanceof AnalysisLevel) {
						AnalysisLevel currentfm = (AnalysisLevel) cs2.getLink()
											.getBusinessObjects().get(0);
									if(currentfm.getFunctionalAnalysisArchitecture() != null&&
											targetfm.getFunctionalAnalysisArchitecture() !=null)
									if (currentfm.getFunctionalAnalysisArchitecture().equals(
											context.getNewObject())
											)
										// Target AnalysisLevel contains business object of new
										// AnalysisFunctionPrototype
										if (targetfm.getFunctionalAnalysisArchitecture().equals(
												context.getNewObject())
												)
											return true;
								}}}}
	...
	public PictogramElement add(IAddContext context) {
		AnalysisFunctionPrototype componentTypeToAdd = (AnalysisFunctionPrototype) context
				.getNewObject();
