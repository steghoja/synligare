...
public class OpenDFADiagramFeature extends AbstractDrillDownFeature {
	private IPeService peService;
	private Collection<Diagram> diagrams;

	public OpenDFADiagramFeature(IFeatureProvider fp) {
		super(fp);
		...
		if (elements.length == 1) {
			...
			if (linkedDiagrams.size() == 0) {
				...
				Diagram diagram = peService.createDiagram(
						"org.eclipse.eatop.examples.graphicaleditor.dfad", "DesignFunctionAllocation diagram", true);
