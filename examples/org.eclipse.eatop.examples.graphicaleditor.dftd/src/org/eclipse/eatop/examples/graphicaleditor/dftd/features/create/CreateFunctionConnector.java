	...
	public Connection create(ICreateConnectionContext context) {
		...
		if (source != null && target != null) {
			...
			if (sAnchor instanceof ContainerShape ) 
			{
				...
				if ( z.getLink().getBusinessObjects().get(0) instanceof DesignFunctionPrototype)
				{
			...
			if (!(sLock == true && tLock ==true)){
				...
				if(sameLock)
				{
					...
					DesignFunctionType model = (DesignFunctionType) getDiagram().getLink().getBusinessObjects().get(0);
