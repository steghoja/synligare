...
public class RemoveFunctionTriggerToFunctionPrototype extends DefaultRemoveFeature {

	public RemoveFunctionTriggerToFunctionPrototype(IFeatureProvider fp) {
		super(fp);
	}
	@Override
	public void preRemove(IRemoveContext context) {
		...
		Assert.isTrue(getBusinessObjectForPictogramElement(element.getEnd().getParent()) 
				instanceof FunctionPrototype, "The start element isn't a FunctionPrototype!");
		FunctionPrototype target = (FunctionPrototype) getBusinessObjectForPictogramElement(element.getEnd().getParent());

		EReference referenceId = Eastadl21Factory.eINSTANCE.getEastadl21Package().getFunctionTrigger_FunctionPrototype();
