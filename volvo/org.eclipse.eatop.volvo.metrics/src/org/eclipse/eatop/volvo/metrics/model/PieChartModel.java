...
public class PieChartModel {
	 ...
	 * @param proportions
	 */
	public PieChartModel(List<Double> proportions) {
	 ...
	 * @param labels
	 */
	public PieChartModel(List<Double> proportions, List<String> labels) {
		checkProportions(proportions);
	 ...
	 */
	public PieChartModel(List<Double> proportions, List<String> labels, 
			List<Color> colors, List<String> toolTips) {
		checkProportions(proportions);
		checkSameSize(proportions, labels);
	 ...
	 */
	public PieChartModel(List<Double> proportions, List<String> labels, 
			List<Color> colors, int diameter, int pos_x, int pos_y) {
		checkProportions(proportions);
		checkSameSize(proportions, labels);
		checkSameSize(proportions, colors);
	 ...
	 * @param proportions
	 */
	private void checkProportions(List<Double> proportions) {
		double sum = 0;
	 ...
	 * @param b
	 */
	private void checkSameSize(List a, List b) {
		if (a.size() != b.size()) {
			throw new IllegalArgumentException("Input arrays not the same size");
		}
	}
	
	private void createPieces(List<Double> proportions) {
	...
	private void createLabels() {
		...
		for (int i = 0; i < labels.size(); i++) {
			ChartLabelModel clm = new ChartLabelModel(labels.get(i), colors.get(i), tooltips.get(i), x, y);
			labelModels.add(clm);
