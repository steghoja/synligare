...
public class VersionPropertyAction implements IObjectActionDelegate{
	private Shell shell;
	...
	public void run(IAction action) {
		...
		try {
			...
			for(EObject selectedEobject : selectedEObjects){				
							...
							for(UserElementType uaType:uaTypes){
								...
								for(UserAttributeDefinition uaDefinition: uaDefinitions){
										if(uaDefinition.getShortName().equals("Version")){
											...
											versionValue = Float.parseFloat(version.getValue());
											showInformation("Version Property",uaType.getShortName().replace("_VersionType", "")+"\n Version: "+ versionValue);
										}								
								}
							}
						}
							...
							for(UserElementType uaType:uaTypes){
								...
								for(UserAttributeDefinition uaDefinition: uaDefinitions){
									...
									if(uaDefinition.getShortName().equals("TargetVersion")){
										EAValue Value = uaDefinition.getDefaultValue();
										final EANumericalValue targetVersion = (EANumericalValue) Value;									
										targetVersionValue = Float.parseFloat(targetVersion.getValue());
									}
				...
				}// end of if ErrorModelType
				

			}
			if(versionExists == false){
				showInformation("Version Property Error", "No version is set for the selected element");
			}
