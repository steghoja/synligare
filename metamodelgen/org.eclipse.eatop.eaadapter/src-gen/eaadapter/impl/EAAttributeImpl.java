...
public class EAAttributeImpl extends EObjectImpl implements EAAttribute {
	...
	public void setAllowDuplicates(Boolean newAllowDuplicates) {
		...
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, EaadapterPackage.EA_ATTRIBUTE__ALLOW_DUPLICATES, oldAllowDuplicates, allowDuplicates));
	}
	...
	public EAElement getElement() {
		if (eContainerFeatureID() != EaadapterPackage.EA_ATTRIBUTE__ELEMENT) return null;
	...
	public boolean validate(DiagnosticChain diagnostic, Map<Object, Object> context) {
				...
				diagnostic.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
						 ...
						 "The attribute's name \""+this.getName()+"\" has leading/trailing whitespaces!",
						 new Object [] { this }));
			}
			isValid = false;			
		}
				...
				diagnostic.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 EaadapterValidator.DIAGNOSTIC_SOURCE,
						 EaadapterValidator.EA_ATTRIBUTE__VALIDATE,
						 "The attribute's name is not well formed!",
						 new Object [] { this }));
			}
			isValid = false;
		}
		...
		if (!this.getStereotype().equals("enum")) {
			
			if (this.getClassifierID() == 0 && this.getType().length() > 0) {
				if (diagnostic != null) {
					diagnostic.add
						(new BasicDiagnostic
							(Diagnostic.ERROR,
							 EaadapterValidator.DIAGNOSTIC_SOURCE,
							 EaadapterValidator.EA_ATTRIBUTE__VALIDATE,
							 "The attribute's classifierID is set to '0', This means, that the attribute's type \""+this.getType()+"\" is Enterprise Architect's internal \""+this.getType()+"\"-type, and not the one from the \"PrimitiveTypes\" Package!",
							 new Object [] { this }));
				}
				isValid = false;
			}
			...
			if (this.getClassifierID() != 0) {
				...
				EAElement referencedClassifier = ValidationHelper.findEAElementByID(this.getClassifierID(), repository);
						...
						diagnostic.add
							(new BasicDiagnostic
								(Diagnostic.ERROR,
								 EaadapterValidator.DIAGNOSTIC_SOURCE,
								 EaadapterValidator.EA_ATTRIBUTE__VALIDATE,
								 "The attribute's classifier with id '"+this.getClassifierID()+"' can not be found!",
								 new Object [] { this }));
					}
					isValid = false;
					
				} else if (!this.getType().equals(referencedClassifier.getName())) {
					//this can happen if the attribute's type is float, but the classifierID points to EAString (maybe an Enterprise Architect Bug)
					if (diagnostic != null) {
						diagnostic.add
							(new BasicDiagnostic
								(Diagnostic.ERROR,
								 EaadapterValidator.DIAGNOSTIC_SOURCE,
								 EaadapterValidator.EA_ATTRIBUTE__VALIDATE,
								 "The attribute's type (\""+this.getType()+"\") and the referenced classifier (\""+referencedClassifier.getName()+"\") don't match!",
								 new Object [] { this }));
					}
					isValid = false;					
				}
	...
	public int eBaseStructuralFeatureID(int derivedFeatureID, Class<?> baseClass) {
		if (baseClass == EAClassifierIDLong.class) {
			switch (derivedFeatureID) {
				case EaadapterPackage.EA_ATTRIBUTE__CLASSIFIER_ID: return AbstracthierachyPackage.EA_CLASSIFIER_ID_LONG__CLASSIFIER_ID;
				default: return -1;
			}
		}
	...
	public int eDerivedStructuralFeatureID(int baseFeatureID, Class<?> baseClass) {
		if (baseClass == EAClassifierIDLong.class) {
			switch (baseFeatureID) {
				case AbstracthierachyPackage.EA_CLASSIFIER_ID_LONG__CLASSIFIER_ID: return EaadapterPackage.EA_ATTRIBUTE__CLASSIFIER_ID;
				default: return -1;
			}
		}
