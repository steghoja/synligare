...
public class EARepositoryImpl extends EObjectImpl implements EARepository {
	...
	public void load(IProgressMonitor monitor) throws Exception {
		...
		SubMonitor elementProgress = progress.newChild(40).setWorkRemaining(this.getModels().size());
		for (EAPackage model : this.getModels()) {														
			//initialize packages
		 ...
		 * must be present before.
		 */	
		SubMonitor createConnectorProgress = progress.newChild(40).setWorkRemaining(this.getModels().size());
		for (EAPackage model : this.getModels()) {		
	...
	public EList<EANamedElement> getEANamedElementByID(long id) {
		...
		for (TreeIterator<EANamedElement> iter = EcoreUtil.getAllProperContents(this.getModels(),true); iter.hasNext();) {
			EANamedElement element = (EANamedElement) iter.next();
			if (element.getId() == id) {
				result.add(element);
			}
	...
	private EAElement createEAElementFromSparxElement(org.sparx.Element e) {
		...
		/**
		 * if an element has sub-elements we are not sure how to manage this.
		 * An Element should not have sub-elements
		 */
